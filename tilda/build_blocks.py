...
# Поле «HTML-код внутрь HEAD» в Тильде режет содержимое примерно на 64 КБ —
# проверено вживую: код обрывался ровно на этой границе, посреди CSS-значения,
...
def write_external_base(head_nodes: list) -> None:
    ...
    css = font_faces_css() + base_css(head_nodes)
    js = runtime_js()
...
def write_manifest(pages: dict[str, list[Block]]) -> None:
    ...
    if STANDALONE:
        ...
        lines += [
            "1. `out/<страница>/00-base.html` → первым блоком на каждой из 4 страниц.",
            "   Проверено вживую: 104 КБ (шрифты, стили, сжатый скрипт) помещаются в блок",
            "   целиком. Поле «HTML-код внутрь HEAD» для этого не годится — там общий бюджет",
            "   на всё, что уже стоит (аналитика, пиксели), и он режет примерно на 64 КБ.",
        ]
    lines += [
